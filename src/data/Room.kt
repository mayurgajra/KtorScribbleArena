...
import com.mayurg.other.matchesWord
...
class Room(
    ...
    suspend fun checkWordAndNotifyPlayers(message: ChatMessage): Boolean {
        if (isGuessCorrect(message)) {
            ...
            broadcastPlayerStates()
            val announcement = Announcement(
                "${message.from} has guessed the word", System.currentTimeMillis(),
    ...
    private suspend fun sendWordToPlayer(player: Player) {
        ...
        word?.let { curWord ->
            drawingPlayer?.let { drawingPlayer ->
                ...
                player.socket.send(Frame.Text(gson.toJson(gson.toJson(gameState))))
            }
        }
        player.socket.send(Frame.Text(gson.toJson(gson.toJson(phaseChange))))
    }
